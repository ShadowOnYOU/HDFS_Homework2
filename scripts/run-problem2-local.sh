...
# 题目二本地测试脚本 - 自定义Combiner和Partitioner
# 使用方法: ./run-problem2-local.sh <输入文件> <输出目录>
...
if [ $# -ne 2 ]; then
    echo "用法: $0 <输入文件> <输出目录>"
    echo "示例: $0 data/simple-test.txt output/problem2-simple"
    exit 1
fi
...
echo "=== 题目二：自定义Combiner和Partitioner（本地测试）==="
...
if [ $? -eq 0 ]; then
    ...
    elif [ -f "$OUTPUT_DIR/part-r-00001" ]; then
        echo "分区0结果:"
        head -10 "$OUTPUT_DIR/part-r-00000" 2>/dev/null || echo "分区0无结果"
        echo "分区1结果:"
        head -10 "$OUTPUT_DIR/part-r-00001" 2>/dev/null || echo "分区1无结果"
        echo "分区2结果:"
        head -10 "$OUTPUT_DIR/part-r-00002" 2>/dev/null || echo "分区2无结果"
        echo "分区3结果:"
        head -10 "$OUTPUT_DIR/part-r-00003" 2>/dev/null || echo "分区3无结果"
    else
        echo "结果文件不存在"
        ls -la "$OUTPUT_DIR/"
    fi
