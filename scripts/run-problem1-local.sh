...
# 题目一本地测试脚本 - HDFS操作与WordCount实现
# 使用方法: ./run-problem1-local.sh <输入文件> <输出目录>
...
if [ $# -ne 2 ]; then
    echo "用法: $0 <输入文件> <输出目录>"
    echo "示例: $0 data/simple-test.txt output/problem1-simple"
    exit 1
fi
...
echo "=== 题目一：HDFS操作与WordCount实现（本地测试）==="
