...
# 题目三运行脚本 - 使用上传的测试数据
# 使用方法: ./run-problem3-with-data.sh <学号> <测试文件名>
...
OUTPUT_PATH="/$STUDENT_ID/homework1/problem3-$(basename "$TEST_FILE" .txt)"
...
if [ $? -eq 0 ]; then
    ...
    echo ""
    echo "=== 优化效果分析 ==="
    echo "查看作业计数器信息以分析优化效果..."
    
    # 保存结果到本地
    LOCAL_OUTPUT="$PROJECT_ROOT/output/problem3-$(basename "$TEST_FILE" .txt)"
