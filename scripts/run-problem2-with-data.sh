...
# 题目二运行脚本 - 使用上传的测试数据
# 使用方法: ./run-problem2-with-data.sh <学号> <测试文件名>
...
OUTPUT_PATH="/$STUDENT_ID/homework1/problem2-$(basename "$TEST_FILE" .txt)"
...
if [ $? -eq 0 ]; then
    ...
    echo "=== 各分区词频统计结果 ==="
    for i in {0..3}; do
        echo "分区 $i (A-F: 0, G-N: 1, O-S: 2, T-Z: 3):"
        hdfs dfs -cat "$OUTPUT_PATH/part-r-0000$i" 2>/dev/null | head -10 || echo "分区 $i 无数据"
        echo ""
    done
    
    # 显示Combiner效果统计
    echo "=== Combiner效果分析 ==="
    echo "查看作业计数器信息以分析Combiner效果..."
    
    # 保存结果到本地
    LOCAL_OUTPUT="$PROJECT_ROOT/output/problem2-$(basename "$TEST_FILE" .txt)"
